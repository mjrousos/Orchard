...
namespace Orchard.Taxonomies.Drivers {
    public class TaxonomyFieldDriver : ContentFieldDriver<TaxonomyField> {
        ...
        private ContentShapeResult BuildEditorShape(ContentPart part, TaxonomyField field, dynamic shapeHelper, TaxonomyFieldViewModel appliedViewModel = null) {
            return ContentShape("Fields_TaxonomyField_Edit", GetDifferentiator(field, part), () => {
                ...
                var appliedTerms = appliedViewModel == null
                    ? GetAppliedTerms(part, field, VersionOptions.Latest).ToDictionary(t => t.Id, t => t)
                    : appliedViewModel.Terms.Where(t => t.IsChecked).ToDictionary(t => t.Id, t => t.ContentItem == null ? GetOrCreateTerm(t, taxonomy.Id, field) : t.ContentItem.As<TermPart>());
                ...
                var viewModel = new TaxonomyFieldViewModel {
                    ...
                    Settings = settings,
                    SingleTermId = appliedViewModel != null ? appliedViewModel.SingleTermId : appliedTerms.Select(t => t.Key).FirstOrDefault(),
        ...
        protected override void Importing(ContentPart part, TaxonomyField field, ImportContentContext context) {
            ...
            var terms = termIdentities.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(context.GetItemFromSession).Where(contentItem => contentItem != null).ToList();
