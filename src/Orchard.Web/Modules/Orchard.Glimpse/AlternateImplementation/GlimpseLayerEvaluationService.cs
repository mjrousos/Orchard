...
namespace Orchard.Glimpse.AlternateImplementation {
    ...
    public class GlimpseLayerEvaluationService : ILayerEvaluationService {
        ...
        private int[] PopulateActiveLayers() {
            var activeLayers = _orchardServices.ContentManager
                .Query<LayerPart>()
                .WithQueryHints(new QueryHints().ExpandParts<LayerPart>())
                .ForType("Layer")
                .List();
            ...
            foreach (var activeLayer in activeLayers) {
                ...
                try {
                    ...
                    var layerRuleMatches = _glimpseService.PublishTimedAction(
                        () => _conditionManager.Matches(currentLayer.Record.LayerRule),
                        (r, t) => new LayerMessage {
                            Active = r,
                        ...
                        TimelineCategories.Layers,
                        "Layer Evaluation",
                        currentLayer.Record.Name
                    ).ActionResult;
